...
def fullLoad(request):
    if request.method == "POST":
        ...
        # Redirect to the fullLoadInvoice view with the created loadId
        messages.success(request, 'Full Load created successfully.')
        return redirect('fullLoadInvoice', pk=con_id)  # Pass the loadId as pk
...
def generate_invoice(request, sender_name):
    # Get filter values from the request
    from_date_str = request.GET.get('from_date')
    to_date_str = request.GET.get('to_date')
    ...
    # Generate a random invoice number between 1000 and 9999 (inclusive)
    invoice_number = random.randint(1000, 9999)

    # Pass the invoice number to the frontend
    return render(request, 'partywise_detail.html', {
        'sender_name': sender_name,
        'consignments': detailed_data,
        'total_pieces': total_pieces,
        'invoice_number': invoice_number  # Pass the random invoice number here
    })
...
def fullLoadInvoice(request, pk):
    try:
        data = Fullload.objects.get(loadId=pk)  # Use get() for a single object
    except Fullload.DoesNotExist:
        data = None
...
from datetime import datetime
from django.shortcuts import render
from .models import Fullload


def fulload_list(request):
    # Get filter values from the request
    from_date_str = request.GET.get('from_date')
    to_date_str = request.GET.get('to_date')
    transpoter = request.GET.get('transpoter')

    # Start building the query
    queryset = Fullload.objects.all()
    ...
    if from_date_str and to_date_str:
        try:
            ...
            return render(request, 'fullLoad_report.html', {
                'error': 'Invalid date format.'
            })


    # Group by sender_name and calculate sum of pieces, total cost, and count of track_id
    fulload_by_transpoter = queryset.values('transport').annotate(
        total=Sum('total'),
        loadId_count=Count('loadId', distinct=True)
    ).order_by('transport')

    # Pass the aggregated data to the template
    context = {
        'fulload_by_transpoter': fulload_by_transpoter,
        'from_date': from_date_str,
        'to_date': to_date_str,
        'transpoter': transpoter, }

    return render(request, 'fullLoad_report.html', context)
...
def get_full_load_data(request):
    # Fetch the data from Fullload model
    # Here, I will fetch all fields (adjust as needed)
    full_loads = Fullload.objects.all().values(
        'route_from', 'route_to', 'date', 'vehicleNo', 'driverName', 'tone',
        'total', 'goods', 'advance', 'balance', 'loadId', 'status', 'paidAmt',
        'haltAmt', 'dedicationAmt', 'finalBalance'
    )

    # Return the data as JSON
    return JsonResponse(list(full_loads), safe=False)
...
def fulload_invoice(request, transport):
    # Get filter values from the request
    from_date_str = request.GET.get('from_date')
    to_date_str = request.GET.get('to_date')

    # Start building the query
    fullloads = Fullload.objects.filter(transport=transport)

    # Check if any records exist
    if not fullloads.exists():
        return render(request, 'fullLoad_detail.html', {'error': 'No full load data found for this loadId.'})

    # Get the transport field from the first record in the queryset
    trans = fullloads.first().transport

    # Fetch transporter details
    data = Transporter.objects.get(companyname=trans)
    ...
    if from_date_str and to_date_str:
        try:
            ...
            # Filter the queryset by the date range
            fullloads = fullloads.filter(date__range=(from_date, to_date))
        except ValueError:
            return render(request, 'fullLoad_detail.html', {
                'error': 'Invalid date format.',
                'loadId': transport,
            })

    # Aggregate details based on loadId
    aggregated_data = fullloads.values(
        'loadId',
        'route_from',
        'route_to',
        'date',
        ...
        total_sum=Sum('total'),  # Use a different name for the annotation
    ).order_by('loadId')

    # Create a list of dictionaries for the final data to be displayed
    detailed_data = []
    for load in aggregated_data:
        descriptions = fullloads.filter(loadId=load['loadId']).values_list('goods', flat=True)
        # Append each description with aggregated data
        detailed_data.append({
            **load,
            'desc_goods': descriptions
        })
